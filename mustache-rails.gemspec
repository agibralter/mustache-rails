# Generated by jeweler
# DO NOT EDIT THIS FILE DIRECTLY
# Instead, edit Jeweler::Tasks in Rakefile, and run the gemspec command
# -*- encoding: utf-8 -*-

Gem::Specification.new do |s|
  s.name = %q{mustache-rails}
  s.version = "0.0.1"

  s.required_rubygems_version = Gem::Requirement.new(">= 0") if s.respond_to? :required_rubygems_version=
  s.authors = ["Aaron Gibralter"]
  s.date = %q{2010-06-27}
  s.description = %q{Rails support for {{Mustache}}.}
  s.email = %q{aaron.gibralter@gmail.com}
  s.extra_rdoc_files = [
    "README.md"
  ]
  s.files = [
    "README.md",
     "Rakefile",
     "VERSION",
     "lib/mustache-rails.rb",
     "lib/mustache/rails.rb"
  ]
  s.homepage = %q{http://github.com/agibralter/mustache-rails}
  s.rdoc_options = ["--charset=UTF-8"]
  s.require_paths = ["lib"]
  s.rubygems_version = %q{1.3.7}
  s.summary = %q{Rails support for {{Mustache}}.}

  if s.respond_to? :specification_version then
    current_version = Gem::Specification::CURRENT_SPECIFICATION_VERSION
    s.specification_version = 3

    if Gem::Version.new(Gem::VERSION) >= Gem::Version.new('1.2.0') then
      s.add_runtime_dependency(%q<mustache>, [">= 0.11.0"])
      s.add_runtime_dependency(%q<rails>, ["~> 2.3.5"])
    else
      s.add_dependency(%q<mustache>, [">= 0.11.0"])
      s.add_dependency(%q<rails>, ["~> 2.3.5"])
    end
  else
    s.add_dependency(%q<mustache>, [">= 0.11.0"])
    s.add_dependency(%q<rails>, ["~> 2.3.5"])
  end
end

